...
# Check if ffmpeg is installed (for video thumbnails)
echo ""
echo -e "${BLUE}[2.5/7]${NC} Checking ffmpeg (for video thumbnails)..."
if ! command -v ffmpeg &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
        ...
        echo -e "${YELLOW}⚠${NC}  Could not install ffmpeg automatically. Video thumbnails will not work."
        echo -e "${YELLOW}⚠${NC}  Install ffmpeg manually if you need video thumbnail support."
    fi
    if command -v ffmpeg &> /dev/null; then
