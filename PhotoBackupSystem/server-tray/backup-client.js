...
async function generateDesktopThumbnail(filePath, fileName, mediaType) {
  const THUMB_WIDTH = 220;
  const THUMB_QUALITY = 60;
  const thumbMime = 'image/jpeg';
  const ext = path.extname(fileName || '').toLowerCase();
  const isVideo = mediaType === 'video' || ['.mp4', '.mov', '.avi', '.mkv', '.m4v', '.3gp', '.webm'].includes(ext);
  const tmpBase = path.join(os.tmpdir(), `photolynk_thumb_${process.pid}_${Date.now()}_${crypto.randomBytes(6).toString('hex')}`);
  const tempFramePath = `${tmpBase}_frame.jpg`;
  ...
  try {
    ...
    if (isVideo) {
      ...
      const image = sharp(tempFramePath, { failOn: 'none' });
      const info = await image.resize({ width: THUMB_WIDTH, withoutEnlargement: true }).jpeg({ quality: THUMB_QUALITY }).toBuffer({ resolveWithObject: true });
      thumbBuffer = info.data;
      thumbW = info.info?.width || null;
      thumbH = info.info?.height || null;
    } else {
    ...
    if (!thumbBuffer || !thumbBuffer.length) return null;

    return {
      plain: new Uint8Array(thumbBuffer),
    ...
    try { if (fs.existsSync(tempFramePath)) fs.unlinkSync(tempFramePath); } catch (e) {}
  }
}
...
// Threshold of 1 bit matches canonical mobile-v2 cross-platform dedup.
...
class DesktopBackupClient {
  ...
  async uploadManifest(manifestId, encryptedManifest, chunkCount, metadata = {}) {
    const baseUrl = this.getBaseUrl();
    ...
    const response = await axios.post(url, {
      ...
      originalSize: typeof metadata.originalSize === 'number' ? metadata.originalSize : null,
      fileHash: metadata.fileHash || null,
      perceptualHash: metadata.perceptualHash || null,
      creationTime: metadata.creationTime || null,
      exifCaptureTime: metadata.exifCaptureTime || null,
      exifMake: metadata.exifMake || null,
      exifModel: metadata.exifModel || null,
      thumbChunkId: metadata.thumbChunkId || null,
      thumbNonce: metadata.thumbNonce || null,
      thumbSize: typeof metadata.thumbSize === 'number' ? metadata.thumbSize : null,
      thumbW: typeof metadata.thumbW === 'number' ? metadata.thumbW : null,
      thumbH: typeof metadata.thumbH === 'number' ? metadata.thumbH : null,
      thumbMime: metadata.thumbMime || null,
    }, {
      headers: {
        'Authorization': `Bearer ${this.token}`,
  ...
  async uploadFile(file, fileIndex, totalFiles, alreadyManifestIds, alreadyFilenames, alreadyBaseFilenames, alreadyFileHashes, alreadyPerceptualHashes, alreadyBaseNameSizes, alreadyBaseNameDates, alreadyBaseNameTimestamps, alreadyExifFull, alreadyExifTimeModel, alreadyExifTimeMake) {
    ...
    const serverHasCanonicalHashMeta = (!!exactFileHash && !!alreadyFileHashes && alreadyFileHashes.size > 0) || (!!perceptualHash && !!alreadyPerceptualHashes && alreadyPerceptualHashes.size > 0);

    if (!serverHasCanonicalHashMeta) {
      if (normalizedFilename && alreadyFilenames && alreadyFilenames.has(normalizedFilename)) {
        console.log(`Skipping ${fileName} - filename already on server (legacy fallback)`);
      ...
      if (baseName && fileModified && alreadyBaseNameTimestamps && alreadyBaseNameTimestamps.has(baseName)) {
        ...
        if (fullTimestamp && alreadyBaseNameTimestamps.get(baseName).has(fullTimestamp)) {
          console.log(`Skipping ${fileName} - base filename + timestamp already on server (legacy fallback)`);
      ...
      if (baseName && fileSize && alreadyBaseNameSizes && alreadyBaseNameSizes.has(baseName)) {
        for (const existingSize of alreadyBaseNameSizes.get(baseName)) {
          ...
          if (diff < 0.20) {
            console.log(`Skipping ${fileName} - base filename + size already on server (legacy fallback, diff=${(diff * 100).toFixed(1)}%)`);
      ...
      if (baseName && fileModified && alreadyBaseNameDates && alreadyBaseNameDates.has(baseName)) {
        ...
        if (dateStr && alreadyBaseNameDates.get(baseName).has(dateStr)) {
          console.log(`Skipping ${fileName} - base filename + date already on server (legacy fallback)`);
    ...
    let thumbMime = null;
    try {
      const thumbnail = await generateDesktopThumbnail(filePath, fileName, this.getMediaType(fileName));
      if (thumbnail && thumbnail.plain && thumbnail.plain.length > 0) {
        const thumbNonce = new Uint8Array(24);
        crypto.randomFillSync(thumbNonce);
        const boxedThumb = nacl.secretbox(thumbnail.plain, thumbNonce, this.masterKey);
        thumbChunkId = crypto.createHash('sha256').update(Buffer.from(boxedThumb)).digest('hex');
        thumbNonceB64 = naclUtil.encodeBase64(thumbNonce);
        thumbSize = thumbnail.thumbSize;
        thumbW = thumbnail.thumbW;
        thumbH = thumbnail.thumbH;
        thumbMime = thumbnail.thumbMime;
        await this.uploadChunk(thumbChunkId, boxedThumb);
